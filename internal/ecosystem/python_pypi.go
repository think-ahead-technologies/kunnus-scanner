// ABOUTME: Shared helpers for every PyPI-family lockfile parser (poetry, pdm, pipfile, uv, requirements).
...
// addPyPIFileHash records sri under purl in out if sri is a well-formed
// sha256: reference. Used by every parser that walks a list of file-hashes.
func addPyPIFileHash(out hashes.Map, purl, sri string) {
	if h, ok := pypiHashFromSRI(sri); ok {
		out.Add(purl, h)
	}
}
